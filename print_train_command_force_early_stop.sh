...
seed=$4
execute=$5
early_stop_val_acc=$6

case "$network" in
        fc)
                case "$type" in 
                        iter)
                                filedir=early_stopped_fc_${early_stop_val_acc}
                                ;;
                        one)
                                filedir=lot_fc_orig
                esac
...
case "$execute" in
    t)
        resman -d "./results/${filedir}/" -r "${orig_dir_name}_seed_${seed}" -t "{runname}" -- python train.py --train_h5 "./data/${data}_train.h5" --test_h5 "./data/${data}_test.h5" --val_h5 "./data/${data}_val.h5" --train_batch_size 60 --num_epochs $num_epochs --eval_every 1 --print_every 1 --save_weights --save_loss --arch $arch --seed $seed --opt adam --lr $lr --mode save_all --large_batch_size $large_batch_size --test_batch_size $small_batch_size --val_batch_size $small_batch_size --early_stop_val_acc $early_stop_val_acc
        ;;
    *)
